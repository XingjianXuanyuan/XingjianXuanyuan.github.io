...
| Frank | $\log\biggl( \frac{1 - e^{-\theta}}{1 - e^{-\theta z}} \biggr)$ | $(-\infty, \infty)$ | $\frac{1 - e^{-\theta z}}{\theta e^{-\theta z}}\log\biggl( \frac{1 - e^{-\theta}}{1 - e^{-\theta z}} \biggr)$ | $1 + 4\biggl( \int_{0}^{\theta} \frac{t}{\theta (e^{t} - 1)}\,\mathrm{d}t - 1 \biggr) / \theta$
