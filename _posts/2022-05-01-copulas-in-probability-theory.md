...
and the **survival function** corresponding to $F(y_{1}, \dots , y_{m})$ is given by $$\Bar{F}(y_{1}, \dots , y_{m}) = \mathbb{P}\{Y_{i} > y_{i}; i = 1, \dots , m\}.$$
...
$$F(x) = \mathbb{P}\{X \leq x\} = \mathbb{P}\{F_{i}^{-1}(U_{i}) \leq x_{i}, 1 \leq i \leq n\} = \mathbb{P}\{U_{i} \leq F_{i}(x_{i}), 1 \leq i \leq n\} = C(F_{1}(x_{1}), \dots , F_{n}(x_{n})),$$
